  ...
  try {
    ...
    if (req.body.address) {
      ...
      if (!user || !user.address || !user.address.address) {
        return res.status(400).json({
          ...
          error: ["No delivery address provided and no default address found for user"]
        });
      }
    ...
    try {
      ...
      if (deliveryData) {
        ...
        console.log(`Using delivery rate: ${deliveryRate} for ${address.country}, ${address.region}`);
      } else {
        console.log(`No delivery rate found for ${address.country}, ${address.region}. Using default rate of ${deliveryRate}.`);
      }
    } catch (error) {
      console.error('Error fetching delivery rate:', error);
    }

    const calculateOrder = orderAmountCalc(populatedCartItems, deliveryRate, discount);
